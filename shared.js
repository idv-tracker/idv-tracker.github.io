...
// onclick等のHTML属性内に書くJS文字列リテラル用。
// escapeHTMLだけでは属性値デコード後に ' がJS文字列を脱出できるため、
// 先にJSエスケープ（\ と '）をかけてからHTMLエスケープする
...
// ===== 定数 =====
const SURVIVORS = ['幸運児', '医師', '弁護士', '泥棒', '庭師', 'マジシャン', '冒険家', '傭兵', '祭司', '空軍', '機械技師', 'オフェンス', '心眼', '調香師', 'カウボーイ', '踊り子', '占い師', '納棺師', '探鉱者', '呪術師', '野人', '曲芸師', '一等航海士', 'バーメイド', 'ポストマン', '墓守', '「囚人」', '昆虫学者', '画家', 'バッツマン', '玩具職人', '患者', '「心理学者」', '小説家', '「少女」', '泣きピエロ', '教授', '骨董商', '作曲家', '記者', '航空エンジニア', '応援団', '人形師', '火災調査員', '「レディ・ファウロ」', '「騎士」', '気象学者', '弓使い', '「脱出マスター」', '幻灯師', '闘牛士', 'マイムアーティスト'];

const HUNTERS = ['復讐者', '道化師', '断罪狩人', 'リッパー', '結魂者', '芸者', '白黒無常', '写真家', '狂眼', '黄衣の王', '夢の魔女', '泣き虫', '魔トカゲ', '血の女王', 'ガードNo.26', '「使徒」', 'ヴァイオリニスト', '彫刻師', 'アンデッド', '破輪', '漁師', '蝋人形師', '「悪夢」', '書記官', '隠者', '夜の番人', 'オペラ歌手', '「フールズ・ゴールド」', '時空の影', '「足萎えの羊」', '「フラバルー」', '雑貨商', '「ビリヤードプレイヤー」', '「女王蜂」', '歯医者'];

const MAPS = ['軍需工場', '赤の教会', '聖心病院', '湖景村', '月の河公園', 'レオの思い出', '永眠町', '中華街', '罪の森'];

const TRAITS = ['リッスン', '異常', '興奮', '巡視者', '瞬間移動', '監視者', '神出鬼没', '移形'];
function buildTraitIconPath(traitName) {
  // src属性に直接埋め込まれるため属性エスケープ（正規データでは無変化）
  return escapeHTML(`traits/trait_${traitName}.PNG`);
}

const RANKS = ['1段', '2段', '3段', '4段', '5段', '6段', '7段', '最高峰'];

function calcWinratePct(wins, losses) {
...
function buildIconPath(charName, type) {
  ...
  // src属性に直接埋め込まれるため属性エスケープ（正規データでは無変化）
  return escapeHTML(`${folder}/${prefix}_${name}.PNG`);
}
